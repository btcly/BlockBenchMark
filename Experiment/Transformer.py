...
# 构建 Transformer 模型
def transformer_model(units, d_model, n_heads, ff_dim, dropout=0.1, name="transformer"):
    inputs = Input(shape=(sequence_length, 1), name="inputs")
    x = inputs
    ...
    return Model(inputs=inputs, outputs=out, name=name)

# 初始化 Transformer 模型
model = transformer_model(units=1, d_model=256, n_heads=4, ff_dim=4, dropout=0.1)

# 编译模型
model.compile(optimizer='adam', loss='mse')

# Reshape 数据以符合 Transformer 输入要求
